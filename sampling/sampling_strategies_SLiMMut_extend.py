...
parser.add_argument("time", help="time (from present)", type = int)
...
OUTSUFFIX=args.out
SEED=args.seed
TIME_TO_SAMPLE = args.time

def midpoint(nts,x,y, sample_from_inds,SAMPLE_SIZE, SEED):
	print("performing mid-point sampling")
	
	x_choose = list(np.nonzero((x > 0.45) & (x < 0.55))[0])
	y_choose = list(np.nonzero((y > 0.45) & (y < 0.55))[0])
	inds_to_choose = np.array(sample_from_inds)[list(set(x_choose) & set(y_choose))]
...
def random_diploid(nts, sample_from_inds, SAMPLE_SIZE, SEED):
	print("performing random-diploid sampling")
	
	np.random.seed(seed=SEED)
	inds = np.random.choice(sample_from_inds, int(SAMPLE_SIZE/2), replace = False) # outputs individual indices
...
print("sampling from", TIME_TO_SAMPLE, "generations ago")
sample_from_inds = [i.individual for i in nts.nodes() if i.time == TIME_TO_SAMPLE]
sample_from_inds = list(set(sample_from_inds))
print("number of individuals found:", len(sample_from_inds))

x = np.array([nts.individual(i).location[0] for i in sample_from_inds])
y = np.array([nts.individual(i).location[1] for i in sample_from_inds])

OUTSUFFIX = OUTSUFFIX + "_" + str(TIME_TO_SAMPLE)

if(args.mid):
	samps, outfile = midpoint(nts,x,y, sample_from_inds,SAMPLE_SIZE, SEED)
	print("number of samples =", len(samps))
	write_output(samps, outfile)
	
if(args.random_diploid):
	samps, outfile = random_diploid(nts,sample_from_inds,SAMPLE_SIZE, SEED)
	write_output(samps, outfile)
